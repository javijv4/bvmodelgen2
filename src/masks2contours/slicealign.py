...
def assign_translations_to_slice(cmrs, slices):
    # Initializing dictionary to save translations
    translations = {}
    for view in cmrs.keys():
        translations[view] = np.zeros([cmrs[view].data.shape[2],2])

    # Saving translations to array
    for n in range(len(slices)):
        trans = slices[n].accumulated_translation
        view = slices[n].cmr.view
...
def save_translations(path, cmrs, slices):
    translations = assign_translations_to_slice(cmrs, slices)

    # Saving translation to file
    for view in cmrs.keys():
        fname = os.path.basename(cmrs[view].fname).split('.')[0]
        np.save(path + fname + '_translation', translations[view])
...
def intersect_two_slices(slice1, slice2, which):
    ...
    slice_ind = np.where(np.abs(dist_plane) < 0.5*slice2.cmr.pixdim)[0]
...
def intersect_slice_normal(slc, translation, normal, origin, which):
    ...
    slice_ind = np.where(np.abs(dist_plane) < 0.5*slc.cmr.pixdim)[0]
...
def slice_intersection_error(translation, ind, slices):
    ...
    for m in range(len(slices)):
        ...
        if slc.cmr.view == 'sa' and slices[m].cmr.view == 'sa': continue # 'sa slices do not intersect'
        if slc.cmr.view == 'la' and slices[m].cmr.view == 'la': weight=5
...
def slice_intersection_error2(translation, ind, slices):
    ...
    for m in range(len(slices)):
        ...
        if 'la' in slices[m].cmr.view:
            la_slices.append(slices[m])
        if 'sa' in slices[m].cmr.view:
...
def find_SA_initial_guess(slices):  # TODO use contours
    for n in range(len(slices)):
        slc = slices[n]
        if slc.cmr.view != 'sa': continue
        points = get_slice_intersection_points(n, slices)

        A, t = nib.affines.to_matvec(slc.cmr.affine)
...
def slice_intersection_error_affine(affine, ind, slices):
    ...
    for m in range(len(slices)):
        weight=1
        # if (m == ind) and (slc.cmr.view == 'sa'): weight=100 # continue   # skip the same slice
        if slc.cmr.view == 'sa' and slices[m].cmr.view == 'sa': continue # 'sa slices do not intersect'
        if slc.cmr.view == 'la' and slices[m].cmr.view == 'la': weight=1
        for contour in slc_xyz.keys():
            if (contour == 'rvendo') and (not slices[m].has_rv): continue
...
def plot_slice_intersection(ind, slices, use_cum_trans=True, use_cum_affine=True):
    ...
    fig = pf.show_point_cloud(slc_xyz, opacity=0.5, size=5, color='blue')
    fig = pf.show_point_cloud(points, opacity=0.5, size=5, fig=fig, color='red')
    fig.show()
